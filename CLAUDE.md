...
- `FluidAudio` (SPM dependency, used for Parakeet) also provides the **speaker diarization** behind Conversation Mode's opt-in speaker split (`SpeakerDiarizationService`). In-pipeline model loads are **cache-only** (never network); downloads happen only from the settings toggle / record-start warm-up — keep it that way.
...
- **conversation-mode** — the multi-source subsystem: architecture, the load-bearing invariants, the correctness traps, persistence + the deferred @Relationship migration.
